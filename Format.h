...
struct Format{
	private:
	static constexpr std::hash<const char*> hasher{};
	
	public:
	Format(const char* const str):name(str),cacheHsh(hasher(str))
		{}
	Format(const Format& f):name(f.name),cacheHsh(f.cacheHsh)
		{}
	Format(Format&& f):name(std::move(f.name)),cacheHsh(f.cacheHsh)
		{}
...
constexpr Format BLANKFORMAT;	//object of Format that represents a bad Format
